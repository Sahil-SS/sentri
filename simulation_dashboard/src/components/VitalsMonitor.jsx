...
export default function VitalsMonitor() {
  ...
  useEffect(() => { vitalsRef.current = currentVitals; }, [currentVitals]);
    ...
    <div style={{ padding: '16px', background: '#e8e8e8' }}>
      <div style={{
        ...
        <div style={{
          ...
          padding: '5px 16px',
          display: 'flex', alignItems: 'center', gap: 24, height: 34,
        }}>
          <Chip label="BED"     value="ICU-1"                                        color="#00ff7f" />
          <Chip label="PATIENT" value={selectedPatient || '---'}                     color="#e8edf2" />
          <Chip label="AGE"     value={v?.age  != null ? `${Math.round(v.age)} yr` : '---'} color="#e8edf2" />
          <Chip label="HOUR"    value={v?.hour != null ? Math.round(v.hour)         : '---'} color="#6b7a8d" />
        </div>
        ...
        <div style={{ display: 'flex', borderTop: '2px solid #1a2332' }}>
          <VitalPanel label="Heart Rate"  value={hr}  unit="bpm"    color="#00ff7f" rangeKey="hr"   />
          <Sep />
          <VitalPanel label="SpO₂"        value={sp}  unit="%"      color="#00e5ff" rangeKey="spo2" />
          <Sep />
          <VitalPanel label="Temperature" value={tmp} unit="°C"     color="#ffd700" rangeKey="temp" />
          <Sep />
          <VitalPanel label="Resp Rate"   value={rr}  unit="br/min" color="#e0e0e0" rangeKey="resp" />
          <Sep />
          <VitalPanel label="MAP"         value={map} unit="mmHg"   color="#39ff14" rangeKey="map"  />
        </div>
      </div>
    </div>
  );
